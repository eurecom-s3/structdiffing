...
import copy
import json
import pipeline.tree_operations as tree_op
...
def diff_node(dict_1: dict, dict_2: dict, differences, path):
    """
...
def diff_simple_attributes(dict_1, dict_2, diff_dict):
    ...
    for key in dict_1.keys():
        if isinstance(dict_1[key], str) or isinstance(dict_1[key], bool) or isinstance(dict_1, int):
            if key == 'name' or key == 'is_visited':
                continue
            if key not in dict_2:
                dict_2[key] = type(dict_1[key])()
...
def recursive_diff(dict_1, dict_2, differences, path):
    ...
    if comparable is None:
        ...
        differences.append({'difference_type': 'deletion', 'name': f'{tree_op.clean_path(path)}', 'field_info': dict_1})
    else:
...
def find_additions(dict_2, differences, path):
    ...
    if dict_2['is_visited'] is False:
        differences.append({'difference_type': 'addition', 'name': f'{tree_op.clean_path(path)}', 'field_info': dict_2})
