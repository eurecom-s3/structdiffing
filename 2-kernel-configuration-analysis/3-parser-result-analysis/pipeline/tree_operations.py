...
def make_fields_dict(obj):
    ...
    """
    if isinstance(obj, dict) and 'fields' in obj.keys() and isinstance(obj['fields'], list):
        fields_dict = {}
...
def flatten_ifdefs_recursive(element, ifdefs):
    ...
    for key in element.keys():
        if isinstance(element[key], list):
            flagged_ifdefs = []
            #first iteration is to opagate is to propagate ifdef names to the bottom of the tree
...
def clean_path(path):
    ...
    """
    return path.replace('fields_dict/', '').replace('/', '.').replace('struct ', '')
